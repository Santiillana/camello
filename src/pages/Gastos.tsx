...
function FormularioGasto({categorias,onGuardado,onCancelar}:{categorias:CategoriaGasto[];onGuardado:()=>void;onCancelar:()=>void}){
  ...
  return <>{borrador.pendiente&&<BorradorPendiente fecha={borrador.pendiente.updated_at} onDescartar={()=>void borrador.descartar()} onContinuar={async()=>{const p=borrador.pendiente;if(!p)return;const paso=await borrador.continuar();Object.assign(p.datos);setPaso(paso);}}/>}<AsistenteTarjetas titulo="Nuevo gasto" tarjetas={tarjetas} onCompletar={guardar} onCancelar={onCancelar} textoFinal={guardando?'Guardando…':'CONFIRMAR GASTO'} pasoInicial={paso} onPasoChange={setPaso} onGuardarBorrador={borrador.guardarAhora} onDescartarBorrador={borrador.descartar}/></>
}
